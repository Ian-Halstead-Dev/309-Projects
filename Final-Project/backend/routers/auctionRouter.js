...
router.post("", authMiddleware, async (req, res) => {
  if (
    ...
    return res
      .status(400)
      .send("Title, startPrice, daysActive, and description are required");
  }
  req.body.startPrice = req.body.startPrice * 100;

  let query =
    "INSERT INTO auctions (title, description, owner, curr_price, days_remaining ) VALUES (?, ?, ?, ?, ?)";
...
router.put("/bid", authMiddleware, async (req, res) => {
  ...
  if (auction.curr_price > bidAmount) {
    return res
      .status(400)
      .send(
        "bidAmount must be greater than the current price: $" +
          (auction.curr_price / 100).toFixed(2)
      );
  }

  let updateQuery =
    "UPDATE auctions SET curr_price = ?, currentWinner = ? WHERE id = ?";
